...
public interface QuestaoEstudadaRepository extends JpaRepository<QuestaoEstudada, Integer> {
    ...
    @Query(value = "SELECT q FROM QuestaoEstudada q WHERE q.estudo.id = :estudoId AND q.nivel = :nivel")
    List<QuestaoEstudada> recuperarQuestaoEstudadasPorEstudo(@Param("estudoId") int estudoId, @Param("nivel") Nivel nivel);
}
